...
export function Comparison({ dict }: { dict: ComparisonDict }) {
  return (
    <section id="comparativa" className="bg-black py-32 lg:py-40 px-6">
      <div className="max-w-[1200px] mx-auto">
            ...
            <table className="w-full min-w-[700px]">
              <thead>
                <tr className="border-b border-[#262626]">
                  <th className="text-left text-sm text-[#737373] font-normal pb-4 pr-4 w-[200px]">{dict.feature}</th>
              ...
              <tbody>
                {dict.features.map((name, i) => (
                  <tr key={name} className={`border-b border-[#1A1A1A] ${i % 2 === 0 ? "" : "bg-[#0A0A0A]/50"}`}>
                    <td className="text-sm text-[#A3A3A3] py-4 pr-4">{name}</td>
                    <td className="text-center py-4 px-4 bg-[#9333EA]/5"><div className="flex justify-center"><CellValue value={featureData[i].zentral} partialLabel={dict.partial} /></div></td>
                    <td className="text-center py-4 px-4"><div className="flex justify-center"><CellValue value={featureData[i].odoo} partialLabel={dict.partial} /></div></td>
                    <td className="text-center py-4 px-4"><div className="flex justify-center"><CellValue value={featureData[i].defontana} partialLabel={dict.partial} /></div></td>
                    <td className="text-center py-4 px-4"><div className="flex justify-center"><CellValue value={featureData[i].monday} partialLabel={dict.partial} /></div></td>
                    <td className="text-center py-4 px-4"><div className="flex justify-center"><CellValue value={featureData[i].zoho} partialLabel={dict.partial} /></div></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <motion.p initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.2 }} className="text-base text-[#A3A3A3] mt-10">
          <span className="text-white font-medium">$199/mes</span> {dict.footer.split("$199/mes")[1] || dict.footer.split("$199/month")[1]}
        </motion.p>
      </div>
    </section>
  );
