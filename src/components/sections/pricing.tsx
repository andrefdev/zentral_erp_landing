  ...
  return (
    <section id="precios" className="bg-[#FAFAFA] py-32 lg:py-40 px-6">
      <div className="max-w-[1200px] mx-auto">
        ...
        <SplitWords text={dict.title} className="text-[28px] sm:text-[36px] lg:text-[40px] font-medium text-black leading-[1.15] tracking-[-0.015em] max-w-3xl mb-16" />
            ...
            return (
              <StaggerItem key={plan.name}>
                <div className={`bg-white rounded-2xl p-8 h-full flex flex-col hover:-translate-y-2 transition-all duration-300 ${featured ? "border-2 border-[#9333EA] relative hover:shadow-xl hover:shadow-[#9333EA]/10" : "border border-[#E5E5E5] hover:shadow-lg"}`}>
                  {featured && (
                    <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#9333EA] text-white text-xs font-medium tracking-wider uppercase px-4 py-1.5 rounded-full">{dict.popular}</span>
                  )}
                  <div className="mb-6">
                  ...
                  <div className="mb-8">
                    ...
                    <span className="text-[#737373] text-base">{dict.perMonth}</span>
                  </div>
                  ...
                  <a href="#" className={`block text-center font-medium py-3.5 rounded-lg transition-all duration-300 ${featured ? "bg-[#9333EA] text-white hover:bg-[#7e22ce]" : "bg-black text-white hover:bg-[#9333EA]"}`}>{dict.cta}</a>
                </div>
              </StaggerItem>
            );
          })}
        ...
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }} className="text-center text-sm text-[#737373]">{dict.promo}</motion.p>
      </div>
    </section>
  );
}
