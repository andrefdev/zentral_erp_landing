...
const plans = [
  {
    ...
    featured: false,
    features: ["Hasta 3 usuarios", "10 GB Drive", "Zentral People básico", "Zentral Work", "Zentral Docs", "Soporte por email"],
  },
  {
    name: "Profesional",
    price: 199,
    ...
    featured: true,
    features: ["Hasta 10 usuarios", "50 GB Drive", "Todos los módulos", "Zentral AI incluido", "Personalización visual", "Soporte prioritario"],
  },
  {
    name: "Empresa",
    price: 349,
    ...
    featured: false,
    features: ["Hasta 25 usuarios", "200 GB Drive", "Todo ilimitado", "API y webhooks", "Onboarding dedicado", "SLA garantizado"],
  },
];
        ...
        <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mb-12">
          {plans.map((plan) => (
            <StaggerItem key={plan.name}>
              <div
                className={`bg-white rounded-2xl p-8 h-full flex flex-col hover:-translate-y-2 transition-all duration-300 ${
                  plan.featured
                    ? "border-2 border-[#9333EA] relative hover:shadow-xl hover:shadow-[#9333EA]/10"
                ...
                <ul className="flex flex-col gap-3 mb-8 flex-1">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-center gap-3">
                      <Check size={16} className={plan.featured ? "text-[#9333EA]" : "text-black"} strokeWidth={2.5} />
